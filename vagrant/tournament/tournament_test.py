...
def testCount():
    ...
    if c != 0:
        raise ValueError("After deletion, countPlayers should return zero. Returns: " + str(c))
...
def testReportMatches():
    ...
    for (i, n, w, m) in standings:
        if m != 1:
            raise ValueError("Each player should have one match recorded." + str(standings) + str(matches))
        if i in (id1, id3) and w != 1:
            raise ValueError("Each match winner should have one win recorded.")
        elif i in (id2, id4) and w != 0:
            raise ValueError("Each match loser should have zero wins recorded." + str(standings) + str(matches))
...
def testPairings():
    ...
    for pair in actual_pairs:
        if pair not in possible_pairs:
            raise ValueError(
                "After one match, players with one win should be paired." + str(pairings))
